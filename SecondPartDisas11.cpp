...
void Instruction_Tree:: Add(InstructionList* & po1, InstructionList* & po2, InstructionList* & po3){
	...
	if (root==NULL){
		...
		printf("instruction1=%s\n", po1->instruction);
		printf("instruction2=%s\n", po2->instruction);
		//printf("instruction3=%s\n", po3->instruction);
		//root=(Instruction_Leaf*)malloc(sizeof(Instruction_Leaf*));
		root=new Instruction_Leaf;
		root->pointer=po1;
		root->depth=0;
		root->valid=1;
		//root->left=(Instruction_Leaf*)malloc(sizeof(Instruction_Leaf*));
		//root->right=(Instruction_Leaf*)malloc(sizeof(Instruction_Leaf*));
		if (po2!=NULL){
		...
		Instruction_Leaf* p1=root;	
		//Add_Leaf(root, 0, po1, po2, po3);
	}
}
/*
...
/*
void Instruction_Tree::CopyCommand(InstructionLeaf* & p, const char* str){
	p->command=(char*)malloc(sizeof(char)*strlen(str));
	int i=0;
	while (str[i]!='\0'){
		p->command[i]=str[i];
		i++;
...
void PE_Reader::PE_ObjectTable(){
	/*
	printf("We are in PE_OBJECT_TABLE\n");
	int origin=0;
	...
	if (!text_section){
		PE_ObjectTable_Crypted();
	}
	*/
}


void PE_Reader::PE_ObjectTable_Crypted(){
	...
	for (int i=0; i<number_of_entries; i++){
		if ((entry_point_rva>=address_mass[i])&&(entry_point_rva<=address_mass[i]+size_mass[i])){
			printf("This fucking section has number=%d\n", i+1);
...
void Stack::Push(const int a){
	...
	p->pointer=(List_of_Return_Addresses*)malloc(sizeof(List_of_Return_Addresses));
	p=p->pointer;
	p->address=a;
	p->pointer=NULL;
	return;
...
void PolyUnpack::DynamicAnalise(const char* FileName, const int key=0){
	...
	int command_size=32;//default was 10!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	*/ 
	...
	for(;;){
		...
		if (p==NULL)
			printf("Dynamic An Before p=NULL\n");
		flag=Compare(p, string, flag, old_eip_pointer, len);
		if (p==NULL)
			printf("Dynamic An After p=NULL\n");
		if(flag==-1){
			//exit(101);
			valid_file=false;
			invalid_instr_kol++;
...
int PolyUnpack::FindNextInstruction(InstructionList* &p, InstructionList* & p2, const char* command, int & iterator){
	printf("FindNextInstruction Iterator=%d\n", iterator);
	...
	*/ 
	if (compare(p->instruction,"call")){
		...
		if (p->instruction[i]=='['){
			...
			if(p->pointer!=NULL){
				int command_number=p->pointer->number;
				if (iterator==0){
		...
		if(compare(p->instruction, "jmp")){
			...
			if (p->instruction[i]=='0'){
				...
				if (FindInstruction(p2, num-0x20000000L-offset)){
					...
					//root.Add(root, p, p->pointer , NULL);
				}
				return 0;
			}
			return 0;
